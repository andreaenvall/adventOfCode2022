	...
	for _, c := range PuzzleInputDay1() {
		...
		for _, v := range c {
			if _, err := strconv.Atoi(string(v)); err != nil {
				stringNumbers += string(v)

				// Loop through map to find matching string number
				for k, _ := range numbers {
					if strings.Contains(stringNumbers, k) {
						stringNumbers = k
					}
				}
				number, ok := numbers[stringNumbers]
				if ok {
					if len(rowarray) == 2 {
						rowarray = rowarray[:len(rowarray)-1]
						rowarray = append(rowarray, strconv.Itoa(number))
					} else {
						rowarray = append(rowarray, strconv.Itoa(number))
					}
					stringNumbers = string(v)
				...
				if len(rowarray) == 2 {
					rowarray = rowarray[:len(rowarray)-1]
					rowarray = append(rowarray, string(v))
				} else {
					rowarray = append(rowarray, string(v))
				}
			}
		...
		if len(rowarray) == 1 {
			rowarray = append(rowarray, rowarray...)
		}

		number := ""
		for _, r := range rowarray {
			number += string(r)
		}
		numbINt, _ := strconv.Atoi(string(number))
		allNumbers = append(allNumbers, numbINt)
	}
	result := 0
